...
const createUser = async function (req, res) {
    try {
        ...
        if (!(/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(email))) {
            return res.status(400).send({
                status: false,
                msg: "email is not is proper format or email is invalid"
            })
        }
        ...
        if (isEmailAlreadyUsed) {
            return res.status(400).send({
                status: false,
                msg: "this email already exist please enter another email"
            })
        }
        ...
        if (isPhoneAlreadyUsed) {
            return res.status(400).send({
                status: false,
                msg: "this phone already exist please enter another phone"
            })
        }
    ...
    try {
        ...
        if (User) {
            ...
            if (Passwordmatch) {
                ...
                let token = jwt.sign({ userId: User._id, exp: iat + (60 * 50) }, "RoomNo-14")
...
const updateUserDetails = async function (req, res) {
    try {
        ...
        if (Object.keys(Data).length == 0 && files.length == 0) {
            return res.status(400).send({
                status: false,
                msg: "for update request body can't empty"
            })
        }
        ...
        if (Data.hasOwnProperty('phone')) {
            if (validator.isValid(phone) && (/^(\+91[\-\s]?)?[0]?(91)?[6789]\d{9}$/.test(phone))) {
                ...
                return res.status(400).send({
                    status: false,
                    msg: "enter valid indian phone for update"
                })
            }
        }
